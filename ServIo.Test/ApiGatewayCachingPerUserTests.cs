...
public class ApiGatewayCachingPerUserTests
{
    ...
    public async Task PerUser_Cache_Is_Isolated()
    {
        ...
        WriteConfig(ns, ctl, new ApiConfiguration{ ApiName = act, CacheLevel = PowNet.Common.CacheLevel.PerUser, CacheSeconds = 30 });
        ...
        var ctxA1 = Ctx(path); AddUser(ctxA1, "alice", 1); await mw.InvokeAsync(ctxA1); ctxA1.Response.Headers["X-Cache"].ToString().Should().Be("MISS");
        ...
        var ctxB1 = Ctx(path); AddUser(ctxB1, "bob", 2); await mw.InvokeAsync(ctxB1); ctxB1.Response.Headers["X-Cache"].ToString().Should().Be("MISS");
        var ctxB2 = Ctx(path); AddUser(ctxB2, "bob", 2); await mw.InvokeAsync(ctxB2); ctxB2.Response.Headers["X-Cache"].ToString().Should().Be("HIT");

        executions.Should().Be(3); // alice first, bob first, plus one difference due to isolation
    }
    ...
    public async Task Cache_Expires_After_Ttl()
    {
        ...
        WriteConfig(ns, ctl, new ApiConfiguration{ ApiName = act, CacheLevel = PowNet.Common.CacheLevel.AllUsers, CacheSeconds = 1 });
        var parser = new TestParser(ns, ctl, act, path);
        int executions = 0;
        RequestDelegate next = async c => { executions++; await c.Response.WriteAsync("VAL" + executions); };
        var mw = new ApiGatewayMiddleware(next, parser: parser);

        var c1 = Ctx(path); AddUser(c1, "user", 10); await mw.InvokeAsync(c1); c1.Response.Headers["X-Cache"].ToString().Should().Be("MISS");
        await Task.Delay(1100);
        var c2 = Ctx(path); AddUser(c2, "user", 10); await mw.InvokeAsync(c2); // TTL likely expired
        c2.Response.Headers["X-Cache"].ToString().Should().BeOneOf("MISS", "HIT"); // non-deterministic timing, assert execution count
